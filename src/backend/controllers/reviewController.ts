...
export const getReviews = async (req: Request, res: Response) => {
  const rawProductId = req.params.product_id ?? req.query.product_id ?? (req.query as any).productId;
  const productId = String(rawProductId ?? '').trim();
  const rawBatas = Number(req.query.batas);
  const batas = Number.isFinite(rawBatas) && rawBatas > 0 ? Math.floor(rawBatas) : 15;
  if (!productId) return errorResponse(res, 'product_id required', 400);
  try {
...
export const getCustomerReviews = async (req: Request, res: Response) => {
  const rawCustomerId = req.params.customer_id ?? req.query.customer_id ?? (req.query as any).customerId ?? (req.params as any).id;
  const customerId = String(rawCustomerId ?? '').trim();
  const rawBatas = Number(req.query.batas);
  const batas = Number.isFinite(rawBatas) && rawBatas > 0 ? Math.floor(rawBatas) : 15;
  if (!customerId) return errorResponse(res, 'customer_id required', 400);
  try {
