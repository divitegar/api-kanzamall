...
export const getResi = async (req: Request, res: Response) => {
  const order_id = req.params.order_id ?? req.query.order_id;
  if (!order_id) return errorResponse(res, 'order_id required', 400);
  try {
    // fetch order info
    const [rows]: any = await pool.query(`SELECT o.*, sd.name as nama_store, sd.telp_owner, sd.alamat, sd.telp_toko, sd.kodepos, z.name as nama_region, ci.name as nama_kota
      FROM sw_order o
    ...
    if (!row) return errorResponse(res, 'order not found', 404);

    const service = (() => {
      if (row.shipping_method?.includes('YES')) return 'YES';
      return 'REG';
    })();
...
export const trackingResi = async (req: Request, res: Response) => {
  const awb = req.params.awb ?? req.query.awb;
  if (!awb) return errorResponse(res, 'awb required', 400);
  try {
...
export const pickup = async (req: Request, res: Response) => {
  ...
  if (!order_id) return errorResponse(res, 'order_id required', 400);
  try {
    const [rows]: any = await pool.query(`SELECT o.*, sd.name as nama_store, sd.telp_owner, sd.alamat, sd.telp_toko, sd.kodepos, z.name as nama_region, ci.name as nama_kota, co.name as nama_negara
      FROM sw_order o
      LEFT JOIN sw_store_dtl sd ON sd.store_id = o.store_id
