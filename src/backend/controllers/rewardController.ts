...
export const storeCairReward = async (req: Request, res: Response) => {
  ...
  try {
    const [result]: any = await pool.query(`INSERT INTO sw_customer_point_reward (customer_id, point_reward_id, jumlah, tanggal, status) VALUES (?, ?, ?, ?, ?)`,
      [data.customer_id, data.point_reward_id, data.jumlah || 0, data.tanggal || new Date(), data.status || 0]
    );
