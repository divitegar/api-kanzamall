...
export const saveOrderHistory = async (req: Request, res: Response) => {
  const data = req.body;
  if (!data) return errorResponse(res, 'body is required', 400);
  try {
...
export const getCustMember = async (req: Request, res: Response) => {
  try {
    const query = `
...
export const getKomisiOrders = async (req: Request, res: Response) => {
...
export const saveKomisi = async (req: Request, res: Response) => {
  const data = req.body;
  if (!data) return errorResponse(res, 'body is required', 400);
  try {
    const [result]: any = await pool.query('INSERT INTO sw_komisi SET ?', [data]);
