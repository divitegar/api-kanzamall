...
export const getStoreWilayah = async (req: Request, res: Response) => {
  const mst_wilayah_id = req.query.mst_wilayah_id as string | undefined;
  ...
  try {
    ...
    try {
      ...
      const [r]: any = await conn.query(`SELECT urut_jarak FROM sw_mst_wilayah WHERE mst_wilayah_id = ? AND status = 1 AND trash = 0`, [mst_wilayah_id]);
      if (!r || r.length === 0) {
...
export const saveOrder = async (req: Request, res: Response) => {
  ...
  if (!data || !jenis) return errorResponse(res, 'data and jenis are required');

  try {
    if (jenis === 'mst') {
      const fields = Object.keys(data);
      const placeholders = fields.map(() => '?').join(', ');
      const params = fields.map((k) => (data as any)[k]);
      const [result]: any = await pool.query(`INSERT INTO sw_order (${fields.join(',')}) VALUES (${placeholders})`, params);
    ...
    if (jenis === 'dtl') {
      ...
      const placeholders = fields.map(() => '?').join(', ');
      const params = fields.map((k) => (data as any)[k]);
      const [result]: any = await pool.query(`INSERT INTO sw_order_product (${fields.join(',')}) VALUES (${placeholders})`, params);
    ...
    if (jenis === 'histori') {
      ...
      const placeholders = fields.map(() => '?').join(', ');
      const params = fields.map((k) => (data as any)[k]);
      const [result]: any = await pool.query(`INSERT INTO sw_order_history (${fields.join(',')}) VALUES (${placeholders})`, params);
...
export const updateOrder = async (req: Request, res: Response) => {
  ...
  if (!data || !order_id) return errorResponse(res, 'data and order_id are required');
  ...
  try {
    ...
    const [result]: any = await pool.query(`UPDATE sw_order SET ${setClause} WHERE order_id = ?`, params);
...
export const getProdukStore = async (req: Request, res: Response) => {
  const product_id = req.query.product_id as string | undefined;
  const store_id = req.query.store_id as string | undefined;
