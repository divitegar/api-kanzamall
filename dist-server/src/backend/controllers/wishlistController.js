...
export const getWishlist = async (req, res) => {
    ...
    try {
      ...
      LIMIT ?
    `;
        const [rows] = await pool.query(query, [customer_id, batas]);
...
export const addWishlist = async (req, res) => {
    ...
    if (!customer_id || !product_id)
        return errorResponse(res, 'customer_id and product_id are required');
    try {
        // Check if already exists
        const [exists] = await pool.query('SELECT * FROM sw_customer_wishlist WHERE customer_id = ? AND product_id = ?', [customer_id, product_id]);
        if (Array.isArray(exists) && exists.length > 0) {
            return successResponse(res, { message: 'Already in wishlist' });
        }
        const [result] = await pool.query('INSERT INTO sw_customer_wishlist (customer_id, product_id, date_added) VALUES (?, ?, ?)', [customer_id, product_id, new Date()]);
...
export const deleteWishlist = async (req, res) => {
    ...
    if (!customer_id || !product_id)
        return errorResponse(res, 'customer_id and product_id are required');
    try {
        const [result] = await pool.query('DELETE FROM sw_customer_wishlist WHERE customer_id = ? AND product_id = ?', [customer_id, product_id]);
