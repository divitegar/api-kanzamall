...
export const getKategoriKonsultasi = async (req, res) => {
    try {
...
export const getJmlKonsultasiKategori = async (req, res) => {
    try {
...
export const getJmlKonsultasiBaru = async (req, res) => {
    const penerima_id = toNum(req.query.penerima_id ?? req.params.penerima_id);
    if (!penerima_id)
        return errorResponse(res, 'penerima_id required', 400);
    try {
        const [rows] = await pool.query('SELECT COUNT(8) as jml FROM sw_konsultasi WHERE dibaca = 0 AND hapus = 0 AND penerima_id = ?', [penerima_id]);
...
export const isKonsultasiOpen = async (req, res) => {
    const user_id = toNum(req.query.user_id ?? req.params.user_id);
    if (!user_id)
        return errorResponse(res, 'user_id required', 400);
    try {
        const [rows] = await pool.query('SELECT * FROM sw_konsultasi WHERE user_id = ? AND status_konsultasi = 0 AND hapus = 0', [user_id]);
...
export const getJmlAntrian = async (req, res) => {
    try {
        const [rows] = await pool.query("SELECT COUNT(8) as jml FROM sw_konsultasi WHERE dibaca = 0 AND penerima_id = 0 AND hapus = 0");
...
export const jawabAntrian = async (req, res) => {
    const id = toNum(req.params.id || req.body.konsul_id);
    const data = req.body;
    if (!id)
        return errorResponse(res, 'konsul_id required', 400);
    try {
...
export const getDataKonsul = async (req, res) => {
...
export const getChatKonsul = async (req, res) => {
...
export const insertKonsul = async (req, res) => {
    const data = req.body;
    if (!data)
        return errorResponse(res, 'body required', 400);
    try {
        const [result] = await pool.query('INSERT INTO sw_konsultasi SET ?', [data]);
...
export const updateKonsul = async (req, res) => {
    const id = toNum(req.params.konsul_id || req.body.konsul_id);
    const data = req.body;
    if (!id)
        return errorResponse(res, 'konsul_id required', 400);
    try {
        const [result] = await pool.query('UPDATE sw_konsultasi SET ? WHERE konsul_id = ?', [data, id]);
...
export const insertChatMessage = async (req, res) => {
    const data = req.body;
    if (!data)
        return errorResponse(res, 'body required', 400);
    try {
        const [result] = await pool.query('INSERT INTO sw_konsultasi_dtl SET ?', [data]);
...
export const updateChatMessageStatus = async (req, res) => {
    const id = toNum(req.params.konsul_id || req.body.konsul_id);
    const data = req.body;
    if (!id)
        return errorResponse(res, 'konsul_id required', 400);
