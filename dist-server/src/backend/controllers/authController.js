...
const cek_kode_kanza_internal = async (kode_kanza) => {
    const query = `
    SELECT telephone 
    FROM sw_customer 
    WHERE kode_kanza = ? AND type_user = 1
  `;
    const [rows] = await pool.query(query, [kode_kanza]);
    return rows;
};
export const adminLogin = async (req, res) => {
    ...
    `;
        ...
        const token = jwt.sign({ id: user.user_id, username: user.username, type: 'admin', group: user.group_name }, JWT_SECRET, { expiresIn: '1d' });
...
export const customerRegister = async (req, res) => {
    ...
    if (!validateRequired(agree))
        return errorResponse(res, 'Syarat dan Ketentuan Harus disetujui');
    ...
    try {
        ...
        const [existing] = await pool.query('SELECT email, telephone FROM sw_customer WHERE email = ? OR telephone = ?', [email, telephone]);
        if (existing.length > 0) {
            if (existing.some((r) => r.email === email)) {
                return errorResponse(res, 'Email sudah Terdaftar dalam database kami, ganti dgn yg lain');
            }
            if (existing.some((r) => r.telephone === telephone)) {
        ...
        // 5. Simpan Customer
        const [result] = await pool.query(`INSERT INTO sw_customer 
      (firstname, email, telephone, hp_distributor, password, code, status, date_added, ip, kode_kanza, type_user) 
      VALUES (?, ?, ?, ?, ?, ?, '1', CURDATE(), ?, ?, '2')`, [name, email, telephone, hp_distributor, hashedPassword, activasi_key, ip, kode_kanza_new]);
